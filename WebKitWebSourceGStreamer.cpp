...
static bool webKitWebSrcStart(WebKitWebSrc* src)
{
    ...
    String  cleanUrl(priv->uri);
    KURL kurl = KURL(KURL(), priv->uri);
    KURL url;
    String scheme = cleanUrl.lower().substring(0,12);
    if(scheme == "http://ccnx1"){
        cleanUrl = "ccnx:/"+cleanUrl.substring(12,kurl.pathEnd());
        url = KURL(KURL(),cleanUrl);
    }else if(scheme == "http://ccnx2"){
        cleanUrl = "ccnx://"+cleanUrl.substring(12,kurl.pathEnd());
        url = KURL(KURL(),cleanUrl);
    }
    else
        url = KURL(KURL(),priv->uri);
