...
void MediaPlayerPrivateGStreamer::load(const String& url)
{
    ...
    KURL kurl(KURL(), url);
    String cleanUrl(url);
    String scheme = cleanUrl.lower().substring(0,6);

    if(scheme == "ccnx://"){
        cleanUrl = "http://ccnx2" + cleanUrl.substring(7,kurl.pathEnd());
        loadNow(cleanUrl);
    }else if(scheme == "ccnx:/"){
        cleanUrl = "http://ccnx1" + cleanUrl.substring(6,kurl.pathEnd());
        loadNow(cleanUrl);
    }
    else
        loadNow(url);
...
void MediaPlayerPrivateGStreamer::loadNow(const String& url)
{
